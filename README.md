...
Once your Spring Boot application is running on port 8080, open these URLs in your browser:

| Service / Tool | Localhost URL | What It Does & How To Use |
| :--- | :--- | :--- |
| **Web Management Portal** | http://localhost:8080/index.html | The customer directory GUI. Add/edit/delete customers, search by city, view all customers, and inspect real-time validation error responses. |
| **Swagger UI (OpenAPI Docs)** | http://localhost:8080/swagger-ui/index.html | Interactive API documentation. Test all GET, POST, PUT, DELETE endpoints with live request/response inspection right in your browser. |
| **OpenAPI 3.0 JSON Spec** | http://localhost:8080/v3/api-docs | Raw OpenAPI schema definition in JSON format (can be imported into Postman or Insomnia). |
| **H2 Database Console** | http://localhost:8080/h2-console | Built-in database browser. Inspect and query the in-memory SQL CUSTOMERS table directly. |

---

### 🔑 H2 Database Login Credentials
When opening http://localhost:8080/h2-console, enter these exact credentials:
